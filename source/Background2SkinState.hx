...
class Background2SkinState extends MusicBeatSubstate
{
	var curSelected:Int;
	...
	override function create()
	{
		...
		curBackground = FlxG.save.data.backgroundTheme2;
		...
		bg1Button = new FlxSprite(368, 621).loadGraphic(Paths.image('fallmen/UI/Menu/Skins/Menus/Interface/Background/bg1Button'));
		bg1Button.scrollFactor.set();
		bg1Button.updateHitbox();
		bg1Button.antialiasing = ClientPrefs.globalAntialiasing;
		add(bg1Button);

		bg2Button = new FlxSprite(644, 621).loadGraphic(Paths.image('fallmen/UI/Menu/Skins/Menus/Interface/Background/bg2ButtonSel'));
		...
		for (i in 0...skinList.length)
		{
			if (FlxG.save.data.backgroundTheme2 == skinList[i])
			{
				curSelected = i;
			}
			else
	...
	override function update(elapsed:Float)
	{
		...
		for (i in 0...skinList.length)
		{
			if(selectedBox.x == 0)
			{
				if (FlxG.save.data.backgroundTheme2 == "default")
				{
					selectedBox.x = skinIconGroup.members[0].x - 7;
					selectedBox.y = skinIconGroup.members[0].y - 7;
					selectedBox.alpha = 1;
				}
				else if (FlxG.save.data.backgroundTheme2 == skinList[i])
				{
		...
		if (!selectedSomethin && BackgroundState.transitioning == false)
		{
					...
					if (FlxG.mouse.justPressed)
					{
						if (SkinsSelState.devSkinMode == false)
						{
							if (skinStatus[curSelected] == 1)
							{
								...
								FlxG.save.data.backgroundTheme2 = skinList[i];
			...
			else if (FlxG.mouse.overlaps(bg1Button))
			{
				if (playHoverSound == true)
				{
					FlxG.sound.play(Paths.sound('scrollMenu'));
				...
				bg1Button.setGraphicSize(Std.int(bg1Button.width * 1.06636501));
				if (FlxG.mouse.justPressed)
				{
			...
			{
				bg1Button.setGraphicSize(Std.int(bg1Button.width * 1));
