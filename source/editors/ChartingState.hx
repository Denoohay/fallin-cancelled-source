...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['Play BF Animations', "Value 1: 'true' will allow note/miss animations for\nplayer 1. 'false' will turn off any note/miss animations\nfor player 1"],
		['Play Opponent Animations', "Value 1: 'true' will allow note/miss animations\nfor player 2. 'false' will turn off any note/miss animations\nfor player 2"],
		['Fall Guy Run', "Value 1: 'true' will allow note/miss animations\nfor player 2. 'false' will turn off any note/miss animations\nfor player 2"]
	];
	...
	**/
	...
	override function create()
	{
		...
		var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('fallmen/MainMenu/menuBGDesat'));
		bg.scrollFactor.set();
		bg.color = 0xFF222222;
		add(bg);
	...
	function addChartingUI() {
		...
		var check_mute_vocals = new FlxUICheckBox(10, 310, null, null, "Mute P1 Vocals (in editor)", 100);
		...
		playSoundBf = new FlxUICheckBox(check_mute_vocals.x, check_mute_vocals.y + 30, null, null, 'Play Sound (Boyfriend notes)', 100,
			function() {
		...
		playSoundDad = new FlxUICheckBox(check_mute_vocals.x + 120, playSoundBf.y, null, null, 'Play Sound (Opponent notes)', 100,
			function() {
		...
		var check_mute_P2vocals = new FlxUICheckBox(playSoundDad.x, check_mute_vocals.y, null, null, "Mute P2 Vocals (in editor)", 100);
		check_mute_P2vocals.checked = false;
		check_mute_P2vocals.callback = function()
		{
			if(vocalsP2 != null)
			{
				var vol:Float = 1;

				if (check_mute_P2vocals.checked)
					vol = 0;
		...
		check_mute_inst = new FlxUICheckBox(voicesVolume.x + 80, 270, null, null, "Mute Instrumental (in editor)", 100);
		...
		tab_group_chart.add(check_mute_P2vocals);
	...
	function loadSong():Void
	{
		...
		var pile:Dynamic = Paths.voicesP2(currentSongName);
		vocalsP2 = new FlxSound();
		if (Std.isOfType(pile, Sound) || OpenFlAssets.exists(pile)) {
			vocalsP2.loadEmbedded(pile);
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput)
		{
			if (FlxG.keys.justPressed.ESCAPE)
			{
				...
				if(vocalsP2 != null) vocalsP2.stop();
			...
			if (FlxG.keys.justPressed.SPACE)
			{
				if (FlxG.sound.music.playing)
				{
					...
					if(vocalsP2 != null) vocalsP2.pause();
				}
				else
				{
					if(vocals != null) {
					...
					if(vocalsP2 != null) {
						vocalsP2.play();
						vocalsP2.pause();
						vocalsP2.time = FlxG.sound.music.time;
						vocalsP2.play();
					}
