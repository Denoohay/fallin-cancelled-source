...
class NoteOffsetState extends MusicBeatState
{
	...
	override public function update(elapsed:Float)
	{
		...
		if(controls.BACK)
		{
			...
			CustomFadeTransition.nextCamera = camOther;
			FlxG.sound.playMusic(Paths.music('freakyMenu'), 0.5, true);
			FlxG.mouse.visible = false;
			BackgroundState.curMenu == "Freeplay";
			BackgroundState.weirdSwitch = true;
	...
	{
		rating.screenCenter();
		rating.x = coolText.x - -466 + ClientPrefs.comboOffset[0];
		rating.y -= 179 + ClientPrefs.comboOffset[1];

		comboNums.screenCenter();
		comboNums.x = coolText.x - -425 + ClientPrefs.comboOffset[2];
		comboNums.y += -71 - ClientPrefs.comboOffset[3];
		reloadTexts();
	}
